...
const aircraftFeatures = [
  { icon: Plane, label: "Long-haul widebody" },
  { icon: Sparkles, label: "Quiet cabin" },
  { icon: Wind, label: "Modern air filtration" },
  { icon: CircleGauge, label: "High-efficiency airframe" },
] as const;
...
const AircraftSummary = ({ aircraft }: Pick<FlightResult, "aircraft">) => (
  ...
  >
    <div>
      <p className="text-caption text-muted-foreground">Aircraft</p>
    ...
    <ul className="grid gap-3 text-body-sm text-muted-foreground sm:grid-cols-2 lg:grid-cols-4">
      {aircraftFeatures.map(({ icon: Icon, label }) => (
        <li
