...
// The calling code is deliberately kept separate from localized display names.
// NANP territories use +1; the local phone number carries any regional prefix.
const callingCodesByCountry: Record<string, string> = {
  ...
  VA: "+379", VC: "+1", VE: "+58", VG: "+1", VI: "+1", VN: "+84", VU: "+678", WF: "+681", WS: "+685", YE: "+967", YT: "+262", ZA: "+27", ZM: "+260", ZW: "+263",
};

const countryCallingCode = (countryCode: string) => callingCodesByCountry[countryCode];

const callingCodeCountries = countryCodes.filter((countryCode) => countryCallingCode(countryCode));

const preferredCountryForCallingCode = (callingCode: string) => {
  if (callingCode === "+1") return "US";
  if (callingCode === "+7") return "RU";
  if (callingCode === "+44") return "GB";
  if (callingCode === "+47") return "NO";
  if (callingCode === "+61") return "AU";
  if (callingCode === "+212") return "MA";
  if (callingCode === "+262") return "RE";
  if (callingCode === "+358") return "FI";
  if (callingCode === "+590") return "GP";
  if (callingCode === "+599") return "CW";
  return callingCodeCountries.find((countryCode) => countryCallingCode(countryCode) === callingCode);
};

export { callingCodeCountries, countryCallingCode, preferredCountryForCallingCode };
