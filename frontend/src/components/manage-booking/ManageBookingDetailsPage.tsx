...
import { ArrowLeft, ArrowRight, CheckCircle2, Plane, Users } from "lucide-react";
...
import { CustomerETicket } from "@/components/manage-booking/CustomerETicket";
...
const ManageBookingDetailsPage = () => {
  ...
  if (!booking) {
    ...
    return (
      <div className="min-h-screen bg-background pb-24 pt-28 print:p-0" data-ticket-page>
        <Container className="print:max-w-none print:p-0">
          <Link
            className="inline-flex min-h-11 items-center gap-2 text-sm text-muted-foreground transition-colors hover:text-foreground focus-visible:ring-2 focus-visible:ring-focus motion-reduce:transition-none print:hidden"
            href="/manage-booking"
          >
            <ArrowLeft aria-hidden="true" className="size-4" />
            {t("manageBooking.findAnother")}
          </Link>
          <header className="border-b border-border pb-8 print:hidden">
          ...
          <section aria-labelledby="journey-heading" className="py-10 print:hidden">
            ...
            <Card className="mt-6 p-6 sm:p-8">
              ...
              <div className="mt-7 grid gap-4 border-t border-border pt-6 sm:grid-cols-2"><p><span className="text-muted-foreground">{t("manageBooking.flightNumber")}</span><br /><strong>{booking.journey.flightNumber}</strong></p><p><span className="text-muted-foreground">{t("manageBooking.cabin")}</span><br /><strong>{cabinLabelKeys[booking.journey.cabin as keyof typeof cabinLabelKeys] ? t(cabinLabelKeys[booking.journey.cabin as keyof typeof cabinLabelKeys]) : booking.journey.cabin.trim() || t("ticket.notAvailable")}</strong></p></div>
            </Card>
          </section>
          ...
          <div className="grid gap-6 lg:grid-cols-2 print:hidden">
            ...
            {booking.extras.length ? <Card className="p-6"><h2 className="text-h3">{t("manageBooking.extras")}</h2><ul className="mt-5 space-y-3">{booking.extras.map((extra) => <li key={`${extra.passengerOrdinal}-${extra.productCode}`}>{t(productLabelKey(extra.productCode))} <span className="text-muted-foreground">· {t("manageBooking.passengerOrdinal", { ordinal: extra.passengerOrdinal })}</span></li>)}</ul></Card> : null}
