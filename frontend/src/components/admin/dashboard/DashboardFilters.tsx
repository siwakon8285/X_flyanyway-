...
export function DashboardFilters({ initial, loading, routes, onApply }: DashboardFiltersProps) {
  ...
  return <form className="exec-filters" data-exec-filters aria-busy={loading} onSubmit={(event) => { event.preventDefault(); apply(draft); }}>
    ...
    <div className="exec-filter-fields">
      ...
      <label>{t("dashboard.provider")}<select className="exec-filter-control" value={draft.provider} onChange={(e) => change("provider", e.target.value as DashboardProvider)}><option value="ALL">{t("dashboard.allProviders")}</option><option value="STRIPE">{t("dashboard.stripe")}</option><option value="MOCK_BITCOIN">{t("dashboard.mockBitcoin")}</option></select></label>
