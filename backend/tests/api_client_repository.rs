...
async fn test_pool() -> PgPool {
    ...
    clean_api_client_fixtures(&pool)
        .await
        .expect("clean API-client TEST fixtures");
    pool
}

async fn clean_api_client_fixtures(pool: &PgPool) -> Result<(), sqlx::Error> {
    sqlx::raw_sql(
        "DELETE FROM api_client_management_audit
         ...
         WHERE api_client_credential_id IN (
             ...
             WHERE client.created_by_staff_user_id IN (SELECT id FROM staff_users WHERE email LIKE '%@api-client-repo.test')
                OR client.updated_by_staff_user_id IN (SELECT id FROM staff_users WHERE email LIKE '%@api-client-repo.test')
         );
    ...
    Fut: Future<Output = ()> + Send + 'static,
{
    ...
    common::run_fixture_body_with_cleanup(body, move || async move {
        clean_api_client_fixtures(&cleanup_pool).await
    })
    .await;
}
...
async fn creates_unique_public_clients_and_returns_only_public_dto_fields() {
    ...
    run_fixture_body(&pool, move || async move {
        let service = ApiClientManagement::new(Arc::new(SqlxApiClientRepository::new(
            pool_for_body.clone(),
        )));
...
async fn canonical_scope_codes_survive_create_detail_persistence_audit_and_edit() {
    ...
    run_fixture_body(&pool, move || async move {
        ...
        for (created, expected_domain, expected_codes) in [
            ...
            assert_eq!(
                persisted_scope_codes(&pool_for_body, &created.client_id).await,
                expected_codes
            );
        ...
        assert_eq!(
            persisted_scope_codes(&pool_for_body, &analytics.client_id).await,
            vec!["flights:read"]
        );
...
async fn lifecycle_is_explicit_and_revocation_preserves_terminal_history() {
    ...
    run_fixture_body(&pool, move || async move {
        ...
        let row_count: i64 =
            sqlx::query_scalar("SELECT COUNT(*) FROM api_clients WHERE client_id=$1")
                .bind(&created.client_id)
                .fetch_one(&pool_for_body)
                .await
                .unwrap();
