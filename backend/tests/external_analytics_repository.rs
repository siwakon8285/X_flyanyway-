...
async fn insert_service(
    ...
    fixture.services.push(service_id);
    fixture.instances.push(instance_id);
...
async fn insert_booking(
    ...
    fixture.holds.push(hold_id);
    fixture.attempts.push(attempt_id);
    ...
    let ticket_id = if let Some(status) = ticket_status {
        ...
        fixture.tickets.push(ticket_id);
        Some(ticket_id)
    } else {
        None
    };
...
async fn cleanup(pool: &PgPool, fixture: &Fixture) {
    let mut tx = pool.begin().await.expect("begin analytics fixture cleanup");
    for ticket_id in &fixture.tickets {
        sqlx::query("DELETE FROM tickets WHERE id=$1")
            .bind(ticket_id)
            .execute(&mut *tx)
            .await
            .expect("delete analytics ticket");
    }
    for attempt_id in &fixture.attempts {
        sqlx::query("DELETE FROM payment_attempt_seats WHERE payment_attempt_id=$1")
            .bind(attempt_id)
            .execute(&mut *tx)
            .await
            .expect("delete analytics finalized seats");
        sqlx::query("DELETE FROM payment_attempts WHERE id=$1")
            .bind(attempt_id)
            .execute(&mut *tx)
            .await
            .expect("delete analytics payment");
    }
    for hold_id in &fixture.holds {
        sqlx::query("DELETE FROM seat_holds WHERE id=$1")
            .bind(hold_id)
            .execute(&mut *tx)
            .await
            .expect("delete analytics hold");
    }
    for seat_id in &fixture.seats {
        sqlx::query("DELETE FROM flight_seats WHERE id=$1")
            .bind(seat_id)
            .execute(&mut *tx)
            .await
            .expect("delete analytics seat");
    }
    for instance_id in &fixture.instances {
        sqlx::query("DELETE FROM flight_instances WHERE id=$1")
            ...
            .execute(&mut *tx)
            .await
            .expect("delete analytics instance");
    }
    for service_id in &fixture.services {
        sqlx::query("DELETE FROM flight_service_cabins WHERE flight_service_id=$1")
            ...
            .execute(&mut *tx)
            .await
            .expect("delete analytics service cabins");
        sqlx::query("DELETE FROM flight_services WHERE id=$1")
            .bind(service_id)
            .execute(&mut *tx)
            .await
            .expect("delete analytics service");
    }
    sqlx::query("DELETE FROM flight_instances WHERE flight_service_id=$1")
        .bind(fixture.reservation_service)
        .execute(&mut *tx)
        .await
        .expect("delete analytics fixture reservation instances");
    sqlx::query("DELETE FROM flight_services WHERE id=$1")
        .bind(fixture.reservation_service)
        .execute(&mut *tx)
        .await
        .expect("delete analytics fixture reservation service");
    tx.commit().await.expect("commit analytics fixture cleanup");
}
...
async fn summary_returns_only_nonfinancial_aggregate_keys() {
    ...
    let mut fixture = Fixture::new(&setup).await;
