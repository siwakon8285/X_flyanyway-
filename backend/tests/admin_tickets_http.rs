...
async fn cleanup(p: &PgPool) -> Result<(), sqlx::Error> {
    sqlx::raw_sql("DELETE FROM booking_cancellations WHERE ticket_id IN(SELECT id FROM tickets WHERE booking_reference LIKE 'XF23%'); DELETE FROM tickets WHERE booking_reference LIKE 'XF23%'; DELETE FROM seat_holds WHERE id IN(SELECT h.id FROM seat_holds h JOIN flight_instances i ON i.id=h.flight_instance_id JOIN flight_services s ON s.id=i.flight_service_id WHERE s.public_id LIKE 'ticket-http-%'); DELETE FROM flight_instances WHERE flight_service_id IN(SELECT id FROM flight_services WHERE public_id LIKE 'ticket-http-%'); DELETE FROM flight_management_audit WHERE flight_service_id IN(SELECT id FROM flight_services WHERE public_id LIKE 'ticket-http-%'); DELETE FROM flight_service_seat_templates WHERE flight_service_id IN(SELECT id FROM flight_services WHERE public_id LIKE 'ticket-http-%'); DELETE FROM flight_service_cabins WHERE flight_service_id IN(SELECT id FROM flight_services WHERE public_id LIKE 'ticket-http-%'); DELETE FROM flight_services WHERE public_id LIKE 'ticket-http-%'; DELETE FROM staff_security_audit WHERE actor_staff_user_id IN(SELECT id FROM staff_users WHERE email LIKE '%@ticket-http.test'); DELETE FROM staff_sessions WHERE staff_user_id IN(SELECT id FROM staff_users WHERE email LIKE '%@ticket-http.test'); DELETE FROM staff_user_roles WHERE staff_user_id IN(SELECT id FROM staff_users WHERE email LIKE '%@ticket-http.test'); DELETE FROM staff_users WHERE email LIKE '%@ticket-http.test';").execute(p).await?;
    Ok(())
}
