...
## Manage Booking security boundary

Manage Booking reads the existing ticket, successful local payment attempt, consumed hold, finalized `payment_attempt_seats`, passenger summary, and persisted Extras. It never contacts Stripe, issues a ticket, finalizes payment, or changes hold/seat state. Lookup uses `POST /api/v1/manage-booking/lookup`; Booking Reference and Last Name are the only credentials and stay in the POST body, outside URLs and application logs. Email is not a lookup credential.

Successful lookup sets a 30-minute `x_fly_manage_booking` authorization cookie signed with the separate `MANAGE_BOOKING_SIGNING_SECRET`. The cookie is HttpOnly, `SameSite=Lax`, restricted to `/api/v1/manage-booking`, and gains `Secure` when `APP_ENV=production`. Its signed payload contains only a non-sequential ticket UUID, expiry, and random nonce—no passenger or contact data. Every successful lookup replaces that cookie with authorization for the exact verified ticket. `/manage-booking` always shows the lookup form and does not read the cookie; `/manage-booking/details` loads authenticated data. Its `/api/v1/manage-booking/current` response includes the signed QR of that same existing ticket, with no separate UUID field. The old `/manage-booking/ticket` page redirects to details. Private follow-up reads use `/api/v1/manage-booking/current` and `/api/v1/manage-booking/current/ticket`; neither accepts a booking UUID in the URL.
